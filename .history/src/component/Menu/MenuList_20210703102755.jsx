...
export default class MenuList extends Component {
  ...
  // })
  render() {
    const IconSet = ((iconstr) => {
      return <i className={iconstr} style={{ width: "1rem", height="1rem", color: "#fff" }}></i>
    })
    const listItems = this.state.menus.map((item, index) => {
      return <Menu.Item key={index.toString()} icon={IconSet(item.menu_icon)}>
